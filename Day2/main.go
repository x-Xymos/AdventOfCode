...
func calculateInput(noun int, verb int, input []int, c chan int, wg *sync.WaitGroup) {
	defer wg.Done()
	input[1] = noun
	input[2] = verb
	for i := 0; i < len(input); i += 4 {
		switch input[i] {
		case 1:
			input[input[i+3]] = add(&input[input[i+1]], &input[input[i+2]])
		case 2:
			input[input[i+3]] = multiply(&input[input[i+1]], &input[input[i+2]])
		case 99:
			if input[0] == 19690720 {
				c <- noun
				c <- verb
